...
const GSheetService ={
   ...
   InvitationParser : async () => {
    try {
      ...
      const event = "german-emirates-club-20th-birthday-party";
      ...
      const filtered = results.filter(x => {
          ...
          const validEmail = email !== "" && email !== "#N/A";
      ...
      // })
            ...
            await sendBulkRegistration(register_list);
 ...
 const sendBulkRegistration = async (forms) => {
  try {
    for (let i = 0; i < forms.length; i++) {
     ...
     const formData = new FormData();
      ...
      const response = await fetch(`http://localhost:5501/registration`, {
        method: "POST",
        headers: { "Content-Type": "application/json",         "X-User-Lang":"english"  },
        body: JSON.stringify(form),
      });
      // const response = await fetch(`${process.env.CLIENT_ORIGIN}/registration`, {
      //   method: "POST",
      //   body: formData, 
      // });
