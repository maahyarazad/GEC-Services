  ...
  <Page size="A4" style={styles.page} wrap>
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
          ...
          <View style={{ flex: 2 }}>
            <Text style={styles.referenceText}>
              {formData.reference.reference_contact_person_name_1}
            </Text>
            <Text style={styles.referenceText}>
              {formData.reference.reference_contact_person_number_1}
            </Text>
            <Text style={styles.referenceText}>
              {formData.reference.reference_contact_person_email_1}
            </Text>
          </View>

          <View style={{ flex: 2 }}>
            <Text style={styles.referenceText}>
              {formData.reference.reference_contact_person_name_2}
            </Text>
            <Text style={styles.referenceText}>
              {formData.reference.reference_contact_person_number_2}
            </Text>
            <Text style={styles.referenceText}>
              {formData.reference.reference_contact_person_email_2}
            </Text>
          </View>
    ...
    <View style={styles.table}>
      ...
      {formData.items.map((item, i) => {
        ...
        return (
          <View
            key={`item-${i}`}
            style={item.deleted ? { height: 0, opacity: 0, overflow: "hidden" } : {}}
          >
            ...
            >
              ...
              <Text style={styles.tableCol}></Text>
              <Text style={styles.tableCol}></Text>
              <Text style={[styles.tableCol, styles.currencyTotalStyle]}>
                ({item.vat || "-"})
              </Text>
            ...
            <View style={styles.tableBodyRow} key={`body-${i}`}>
              <Text style={styles.tableBodyText} flex={6}>
                <Text style={styles.body_row}>{item?.body || " "}</Text>
              </Text>
            </View>
          </View>
        );
    ...
    <View style={styles.bankDetail}>
      ...
      <View
        style={[
          styles.bankRow,
          formData?.currency?.currency_enable && { marginTop: 10 },
        ]}
      >
        <Text
          style={[
        ...
        >
          AED {formData?.currency?.currency_rate} = 1.00{" "}
          {formData?.currency?.currency_symbol}
        </Text>
      </View>
    </View>
        ...
        <View style={styles.totalRowDefault}>
          ...
          <View style={styles.totalRow}>
            <View style={styles.flexEnd}>
              ...
              >
                {`${formData.currency.currency_symbol} ${(Number(positiveTotal) +
                  Number(negativeTotal)).toFixed(2)}`}
              </Text>
              <Text
                style={[
                  styles.currencyTotalStyle,
              ...
              >
                {`(AED ${((Number(positiveTotalExchange) || 0) -
                  (Number(negativeTotalExchange) || 0)).toFixed(2)})`}
              </Text>

              {/* Non currency_enable case */}
              <Text
                style={[
                  !formData?.currency?.currency_enable
                    ? { textAlign: "right" }
    ...
    <View style={styles.paymentTerms}>
      ...
      >
        {formData.payment_terms.line2 || ""}
      </Text>

      {/* Signature section */}
<View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 40 }}>
  {/* Left signature */}
  <View style={{ flex: 1, paddingRight: 10 }} wrap={false}>
    <View
      style={{
        display: formData.payment_terms.signature_left_name !== "" ? 'flex' : 'none',
      }}
    >
  ...
  <View style={{ flex: 1, paddingLeft: 10, marginBottom: 20 }}>
    <View
      style={{
        display: formData.payment_terms.signature_right_name !== "" ? 'flex' : 'none',
      }}
    >
    ...
    {/* Signature section */}
    <View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 40 }}>
      <View style={{ flex: 1, paddingRight: 10, marginBottom: 20 }}></View>

      {/* Bottom-right signature */}
      <View style={{ flex: 1, paddingLeft: 10 }}>
        <Text style={styles.signatureText}>Understood, agreed, and accepted:</Text>
      </View>
    </View>
...
<View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 40 }}>
  ...
  <View style={{ flex: 1, paddingLeft: 10, marginBottom: 20 }}>
    <View
      style={{
        display: formData.payment_terms.signature_bottom_right_name !== "" ? 'flex' : 'none',
      }}
    >
      <View style={styles.signatureLine} />
      <Text style={styles.signatureText}>
