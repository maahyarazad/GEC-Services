    ...
    { field: 'id', headerName: 'ID', width: 70 },
    {
        ...
        renderCell: (params) =>
            params.row.change_password_required ? (
                ...
                title="User has not updated the temporary password yet">
                    <FaExclamation color="red" size={20} />
                </Tooltip>
            ) : (
                <Tooltip componentsProps={config.tooltip_config}
                title="User has successfully updated their password">
    ...
    const fetchData = useCallback(
        async (paginationModel, sortModel = [], filterModel = { items: [] }) => {
            ...
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_SERVERURL}/payment?${queryParams}`);
